...
def get_package (db_con, dr_num):
    with db_con.cursor() as cursor:
        ...
        if num_row != 0:
            ...
            return True, package_name, package_number
...
if __name__ == "__main__":
    ...
    try:
        ...
        #output_log = os.path.join(test_path, "test.csv")
        ...
        with darts_con.cursor() as cursor:
            ...
            with open(output_log, 'w') as f:
                ...
                header = ['DR Number', 'Priority', 'Abstract', 'Status', 'Package Name', 'Releases Shipped']
                ...
                for row in results:
                    ...
                    fixed, package_name, versionstring = get_package (darts_con, str(row[0]))
                    ...
                    if fixed:
                        ...
                        sqlrow.append("Not Yet Shipped")
                        if str(row[3]) == 'WAIT' or str(row[3]) == 'OPEN':
                            sqlrow.append("Still Debugging")
                        if str(row[3]) == 'BUILD':
                            sqlrow.append("Build in progress, check back tomorrow")
                        if str(row[3]) == 'COMPLETE' or str(row[3]) == 'CLOSED':
                            sqlrow.append("DR completed but no build, please contact the GSC")
