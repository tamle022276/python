#Sample run: python tpt_python.py --database_name=hela --user_name=sitq_ldi_pll_user1 --user_password=sitq_ldi_pll_user1
import sys
import os
import shutil
import time
...
if __name__ == "__main__":
	...
	# Join directory path and dump name
        directory_path = os.path.join(tpt_main_output, "data_file")
        dump_file_name = user_name + "_" + str(get_uuid)
        ...
        logging.info(" * TPT python output location: %s", tpt_python_output)
        logging.info(" * TPT export output location: %s", tpt_export_log)
        logging.info(" * TPT load output location: %s", tpt_load_log)

       
        # Start tpt export
        logging.info("Start TPT export")
        start_tracking('tpt_export', tpt_export_log, export_uuid_id)
        logging.info("Run TPT export command:")
        logging.info(" * tpt export file: %s", tpt_export_file)
        logging.info(" * tpt export log: %s", tpt_export_log)
        run_tpt(tpt_export_file, export_uuid_id, tpt_export_log, directory_path, database_name, user_name, user_password, dump_file_name)
        ...
        # Start tpt load
        logging.info("Start TPT load")
        start_tracking('tpt_load', tpt_load_log, load_uuid_id)
        logging.info("Run TPT Load command:")
        logging.info(" * tpt load file: %s", tpt_load_file)
        logging.info(" * tpt load log: %s", tpt_load_log)
        run_tpt(tpt_load_file, load_uuid_id, tpt_load_log, directory_path, database_name, user_name, user_password, dump_file_name)


        # Scan the tpt load log
