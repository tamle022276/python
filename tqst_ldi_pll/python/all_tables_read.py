# Author: tl151006
# File: all_tables_read.py
# Date Published: 04/12/2016
# Purpose: Run ALL tables read for LDI and PLL test
# Usage: python all_tables_read.py --database_name=db_name --user_name=db_user --user_password=db_password --ignore_error=error_1,error_2,error_n
...
def setup_argparse():
    ...
    parser.add_argument("--all_read_clone", required=True, help="all_read_clone for this run")
...
def exit(ok=True):
    if ok is True or ok == 0:
        logging.info("================= End: ALL Tables Read Success!] =================")
        sys.exit(0)
    else:
        logging.info("================== End: All Tables Read Fail, Please Check Out ERROR Messages Above!] ==================")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        udaExec = teradata.UdaExec (appName="ALL_READ", version="1.0", logConsole=True)
        ...
        all_read_clone = int(args.all_read_clone)
        ...
        # Set logs variables
        all_tables_read = os.path.join(scriptPath, "output", dbs_name, run_timestamp, "latest")
        ...
        if org_all_read_clone == 1:
            old_bteq_log = user_name + "_all_tables_read_loop_" + str(prior_run_id) + "_clone.log"
            ...
            while org_all_read_clone != 0:
                old_bteq_log = user_name + "_all_tables_read_loop_" + str(prior_run_id) + "_clone" + str(org_all_read_clone) + ".log"
                old_bteq_out_file = os.path.join(all_tables_read, old_bteq_log)
                tdtestpy.delete_one_file(old_bteq_out_file) 
                org_all_read_clone -= 1 
        
        old_testuser = user_name + "_all_read_loop_" + str(prior_run_id) + "_python.log"
        ...
        test_log_name = os.path.join(all_tables_read, testuser)        
        ...
        if not tdtestpy.run_bteq_parallel(bteq_in_file, bteq_out_file, dbs_name, user_name, user_name, \
                                          bteq_ignore_errors, all_read_clone, faileddir, passed_dir):
